...
io.on('connection', (socket) => {
  ...
  socket.on('authenticate', (userData: { userId: number; email: string }) => {
    ...
    console.log(`👤 User ${userData.email} (ID: ${userData.userId}) authenticated on socket ${socket.id}`);
  });
  ...
  socket.on('seat:lock', async (data: { seatId: number; seatNumber: string; showId: string }) => {
    try {
      ...
      console.log(`🔒 Seat lock request: ${data.seatNumber} by user ${user.userId} in show ${data.showId}`);
      ...
      if (result.success) {
        ...
        console.log(`✅ Seat ${data.seatNumber} locked successfully for user ${user.userId}`);
      } else {
        ...
        console.log(`❌ Failed to lock seat ${data.seatNumber}: ${result.message}`);
      }
    } catch (error) {
  ...
  socket.on('seat:unlock', async (data: { seatNumber: string; showId: string }) => {
    try {
      const user = socket.data.user;
      if (!user) {
        socket.emit('error', 'User not authenticated');
      ...
      console.log(`🔓 Seat unlock request: ${data.seatNumber} by user ${user.userId} in show ${data.showId}`);
      ...
      if (result.success) {
        ...
        console.log(`✅ Seat ${data.seatNumber} unlocked successfully by user ${user.userId}`);
      } else {
        ...
        console.log(`❌ Failed to unlock seat ${data.seatNumber}: ${result.message}`);
      }
    } catch (error) {
