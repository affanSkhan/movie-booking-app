...
async function updateSeatStatusEnum() {
  try {
    ...
    const currentValues = await pool.query("SELECT unnest(enum_range(NULL::seat_status)) as current_values");
    console.log('Current enum values:', currentValues.rows.map(row => row.current_values));
    
    // Check if 'locked' already exists
    const lockedExists = currentValues.rows.some(row => row.current_values === 'locked');
    ...
    const updatedValues = await pool.query("SELECT unnest(enum_range(NULL::seat_status)) as updated_values");
    console.log('Updated enum values:', updatedValues.rows.map(row => row.updated_values));
