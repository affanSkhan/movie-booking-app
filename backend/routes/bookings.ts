...
router.post("/confirm", authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`🎫 Confirming booking for user ${userId}:`, {
      showId,
      seatNumbers,
      paymentId,
      orderId,
    ...
    try {
      ...
      const bookResult = await seatLockingService.bookSeats(
        seatNumbers,
        showId,
        userId,
      );
...
router.post("/payment-failed", authenticateToken, async (req, res) => {
  try {
    ...
    console.log(
      `💳 Payment failed for user ${userId}, unlocking seats:`,
      seatNumbers,
    );

    // Unlock seats after failed payment
    await seatLockingService.unlockSeatsAfterFailedPayment(
      seatNumbers,
      showId,
      userId,
    );
