...
export const getSeatsByShow = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (result.rows.length === 0) {
    console.log(`No seats found for show ${showId}, creating default 10x10 grid`);
    ...
    const seatRows = Array.from({ length: 10 }, (_, i) => String.fromCharCode(65 + i)); // A, B, C, ..., J
...
export const selectSeat = asyncHandler(async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
...
export const deselectSeat = asyncHandler(async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const { showId, seatNumber } = req.body;
  const userId = req.user?.userId;
...
export const createSeatsForShow = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const seatRows = Array.from({ length: rows }, (_, i) => String.fromCharCode(65 + i)); // A, B, C, ...
...
export const cleanupExpiredLocks = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
