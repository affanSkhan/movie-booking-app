...
export const getShowsByMovie = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (movieId === "all") {
      ...
      result = await pool.query(
        "SELECT * FROM shows WHERE movie_id = $1 ORDER BY show_time",
        [movieId],
      );
    }
...
// Get show by ID
export const getShowById = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
...
export const createShow = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
...
export const updateShow = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const { movie_id, show_time, screen } = req.body;
